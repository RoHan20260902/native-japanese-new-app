...
for v in views:
 ...
 D.append(dict(id=v['id'],sourceId=v['sourceId'],title=v['title'],group=v['group'],state=(v['state'] if v['state']=='页面概览' else '下方内容 '+v['id'].rsplit('-',1)[1]),image='data:image/png;base64,'+base64.b64encode((root/v['image']).read_bytes()).decode(),annotations=sorted(anns,key=lambda a:a['id'])))
for view in D:

...
let result=out;if(process.env.ONLY_SCREENS){const prior=JSON.parse(fs.readFileSync(path.join(root,'captures.json')));result=specs.flatMap(s=>out.some(v=>v.sourceId===s.id)?out.filter(v=>v.sourceId===s.id):prior.filter(v=>v.sourceId===s.id))}const excludedPath=path.join(root,'excluded-views.json');const excluded=fs.existsSync(excludedPath)?JSON.parse(fs.readFileSync(excludedPath)):[];result=result.filter(v=>!excluded.includes(v.id));fs.writeFileSync(path.join(root,'captures.json'),JSON.stringify(result,null,2));fs.writeFileSync(path.join(root,'missing.json'),JSON.stringify(missing,null,2));console.log('DONE',out.length,'views. Missing:',missing);await b.close()})().catch(e=>{console.error(e);process.exit(1)});
